...
/* Bloc compteur d\'utilisateurs dans le panneau gauche */
.user-counter{
	width:100%;
	...
	<div class="card">
		<div class="left">
			<div class="left-inner">
				...
				<div class="brand">
					<h1>Invitation<br>Wizarr</h1>
					<p>Utilisez ce lien d\'invitation pour inviter une personne ou récupérer votre accès au serveur multimédia.</p>
				</div>

				<!-- Compteur d\'utilisateurs — mis à jour dynamiquement par le JS -->
				<div class="user-counter" id="user-counter" style="display:none;">
					<div class="user-counter-label">Membres</div>
				...
				<div class="footer-note">
					Le code affiché ici est automatiquement vérifié lors du chargement de cette page.
				</div>
			</div>
		</div>
		...
		<div class="right">
			<div id="app">
				<div class="loading">
					<div class="status status-loading"><span class="status-dot"></span>En attente</div>
					<h2 class="title">Préparation de l\'invitation</h2>
					<div class="subtitle">Le plugin récupère le code actif ou en crée un nouveau si nécessaire.</div>
...
/* Calcule le nombre de jours entiers restants entre maintenant et une date ISO */
...
function localizeApiMessage(message) {
	var map = {
		"Plugin disabled":              "Le plugin est désactivé",
		"Automatic mode is disabled":   "Le mode automatique est désactivé",
		"Automatic mode disabled":      "Le mode automatique est désactivé",
		"Configuration missing":        "Configuration manquante",
		"Invalid Wizarr response":      "Réponse Wizarr invalide",
		"Invalid plugin response.":     "Réponse invalide du plugin.",
		"Network error":                "Erreur réseau",
		"Access denied":                "Accès refusé",
		"Display file not found":       "Fichier display introuvable",
		"User limit reached":           "Le serveur est actuellement complet. Réessayez plus tard."
	};
	return map[message] || message || "Erreur";
}
...
function copyText(text, feedbackId) {
	...
	navigator.clipboard.writeText(text).then(function () {
		...
		if (el) {
			el.textContent = "Copié dans le presse-papiers";
		...
		var el = document.getElementById(feedbackId);
		if (el) {
			el.textContent = "Copie impossible";
			el.classList.add("show");
			setTimeout(function () { el.textContent = ""; el.classList.remove("show"); }, 1800);
		}
	});
...
/* Met à jour le bloc compteur dans le panneau gauche */
...
/* Charge les deux API en parallèle */
Promise.all([
...
.then(function(results) {
	...
	/* Affiche le compteur même en cas d\'erreur d\'invitation */
	if (statsData && statsData.response && statsData.response.data) {
		renderUserCounter(statsData.response.data);

		/* Limite atteinte : bloque l\'affichage de l\'invitation même si le cache est valide */
		if (statsData.response.data.limit_reached) {
			app.innerHTML =
				"<div class=\"status status-error\"><span class=\"status-dot\"></span>Complet</div>" +
				"<h2 class=\"title\">Serveur actuellement complet</h2>" +
				"<div class=\"subtitle\">Toutes les places sont prises pour le moment.</div>" +
	...
	if (!inviteData || !inviteData.response) {
		app.innerHTML =
			"<div class=\"status status-error\"><span class=\"status-dot\"></span>Erreur</div>" +
			"<div class=\"error-box\">Réponse invalide du plugin.</div>";
		return;
	}

	/* Limite atteinte : affiche l\'erreur, pas d\'invitation */
	...
	app.innerHTML =
		"<div class=\"status status-valid\"><span class=\"status-dot\"></span>Valide</div>" +
		"<h2 class=\"title\">Ton invitation est prête</h2>" +
		"<div class=\"subtitle\">Utilise le lien direct ou partage simplement le code ci-dessous.</div>" +

		"<div class=\"grid\">" +

			"<div class=\"panel\">" +
				"<div class=\"panel-label\">Code d\'invitation</div>" +
				"<div class=\"code\">" + escapeHtml(code) + "</div>" +
				"<div class=\"small-muted\">Ce code est déjà inclus dans le lien d\'invitation juste en dessous.</div>" +
			"</div>" +

			"<div class=\"panel\">" +
				"<div class=\"panel-label\">Lien d\'invitation</div>" +
				"<div class=\"inline-row\">" +
					"<div class=\"inline-grow\">" +
						(url
							...
							: "<div>-</div>") +
					"</div>" +
					"<button class=\"btn\" id=\"copy-link-btn\" type=\"button\">Copier le lien</button>" +
				"</div>" +
				"<div class=\"small-muted\">Utilise ce lien complet pour partager directement l\'invitation Wizarr.</div>" +
		...
		"<div class=\"two-cols\">" +
			"<div class=\"panel\">" +
				...
				"<div class=\"small-muted\">Heure locale</div>" +
			"</div>" +
		"</div>" +

		"<div class=\"help\">Utilisez ce lien pour inviter une personne ou pour récupérer un accès au serveur multimédia. Le lien ci-dessus peut être copié en un clic.</div>";
...
.catch(function(){
	document.getElementById("app").innerHTML =
		"<div class=\"status status-error\"><span class=\"status-dot\"></span>Erreur</div>" +
		"<div class=\"error-box\">Impossible de charger le code d\'invitation.</div>";
});
</script>
</body>
</html>';
